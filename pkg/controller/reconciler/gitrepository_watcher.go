...
func (r *GitRepositoryWatcher) SetupWithManager(mgr ctrl.Manager) error {
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &radappiov1alpha3.DeploymentTemplate{}, rootFileNameField, deploymentResourceRootFileNameIndexer); err != nil {
		return err
	}
...
func (r *GitRepositoryWatcher) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, f := range files {
		...
		if extension == ".bicep" {
			...
			// TODO: string needs to be converted to map[string]string
			...
			// r.createOrUpdateDeploymentTemplate(ctx, deploymentTemplateName, template, string(providerConfig), parameters)
		}
	}
...
func (r *GitRepositoryWatcher) createOrUpdateDeploymentTemplate(ctx context.Context, fileName, template, providerConfig, rootFileName string, parameters map[string]string) {
	logger := ucplog.FromContextOrDiscard(ctx)
